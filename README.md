...
## 💡 교감 텍스트(Ground Truth) 안내
> 교감(校勘): 여러 가지 판본이나 원고를 서로 비교하여 글자의 잘못을 바로잡는 걸 의미
> ground-truth 옵션은 OCR 인식 결과의 정확도(성능 평가)를 비교하기 위해 마련된 기능입니다. 
> 따라서 `data/ground_truth/` 폴더는 빈 폴더(`.gitkeep`) 상태로 유지되며,
> 성능 평가 기능을 사용하시려면 사용자가 직접 정답 텍스트 파일을 해당 경로에 준비해야 합니다.


`--ground-truth` 옵션을 실제로 쓰려면:
